...
5. **视觉验收三条（2026-09 实战固化——同一段话曾在 9 次派单里手写）**：
   - 截图**先落盘本地路径**再交给 agent 读，禁止传 URL（Read 类工具只吃本地文件路径）；
   - 细粒度比对（图标、指示器、1px 线）统一 **3× 最近邻放大**后再看，vision 对小元素会漏检，必要时用深色像素扫描兜底确认「画没画出来」；
   - 裁剪/放大坐标必须标注**设计空间换算比**（例：截图 540×960 = 设计 720×1280 的 0.75），并记住桌面坐标 = 截图坐标 × 显示缩放（本机 2.0）。
6. **进程自检禁用裸 `pgrep -f`**：模式会命中 agent 自己命令行里的同名字样（连带自身的搜索/ugrep 进程），制造「杀不死的多实例」「输出黑洞」假象。用 `pgrep -af <pattern>` 后显式排除自身 pid，或 `ps -eo pid,args | grep -v $$`；杀沙箱一律用精确 PID，勿碰用户实例。
