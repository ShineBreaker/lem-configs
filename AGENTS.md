...
- **悬停链调错符号（2026-09-10 源码核对+探针实证）**：`LSP-HOVER` 是零参交互命令（`define-command lsp-hover () ()`，内部 `display-message` 弹窗），传 point 调每次抛参错被 `ignore-errors` 吞掉——旧 hover 增强从未生效过。取文档调未导出的 `TEXT-DOCUMENT/HOVER`（1 参 point，返 markdown-buffer；`vs$` 双包回退 `:lem-lsp-mode` → `:lem-lsp-mode/lsp-mode`），经 `UPDATE-HOVER-OVERLAY` → `FIND-OVERLAY-THAT-CAN-HOVER` → `SET-HOVER-MESSAGE` 显示；它是同步 LSP 请求，同点位 300ms 节流（`vs-hover-throttled-p`），无 LSP 时回 NIL 穿透。
